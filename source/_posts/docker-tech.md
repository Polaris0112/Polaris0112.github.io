...
date: 2018-03-18
categories: Docker核心技术与实现原理
tags: 
- Docker

---
本帖子记录的是Docker 核心技术与实现原理
...
![docker-core-techs](/images/docker-core-techs.png)
...
![multiple-servers-on-linux](/images/multiple-servers-on-linux.png)
...
![linux-processes](/images/linux-processes.png)
...
![docker-process-group](/images/docker-process-group.png)
...
![docker-network-topology](/images/docker-network-topology.png)
...
![docker-network-forward](/images/docker-network-forward.png)
...
![container-network-model](/images/container-network-model.png)
...
![libcontainer-filesystem](/images/libcontainer-filesystem.png)

想要正常启动一个容器就需要在 rootfs 中挂载以上的几个特定的目录，除了上述的几个目录需要挂载之外我们还需要建立一些符号链接保证系统 IO 不会出现问题。

![libcontainer-symlinks-and-io](/images/libcontainer-symlinks-and-io.png)
...
![docker-shared-resources](/images/docker-shared-resources.png)
...
![cgroups-inheritance](/images/cgroups-inheritance.png)

Linux 的 CGroup 能够为一组进程分配资源，也就是我们在上面提到的 CPU、内存、网络带宽等资源，通过对资源的分配，CGroup 能够提供以下的几种功能：

![cgroups-features](/images/cgroups-features.png)
...
![linux-cgroups](/images/linux-cgroups.png)
...
![docker-container-layer](/images/docker-container-layer.png)

当镜像被 `docker run` 命令创建时就会在镜像的最上层添加一个可写的层，也就是容器层，所有对于运行时容器的修改其实都是对这个容器读写层的修改。

容器和镜像的区别就在于，所有的镜像都是只读的，而每一个容器其实等于镜像加上一个可读写的层，也就是同一个镜像可以对应多个容器。

![docker-images-and-container](/images/docker-images-and-container.png)
...
![docker-aufs](/images/docker-aufs.png)
...
![docker-filesystems](/images/docker-filesystems.png)
...
![docker-storage-driver](/images/docker-storage-driver.png)
